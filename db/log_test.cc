 ...
 public:
  LogTest()
      : reading_(false),
        ...
        reader_(new Reader(&source_, &report_, true, 0)) {}
  ~LogTest() {
    delete writer_;
    delete reader_;
  }
  ...
  void CheckOffsetPastEndReturnsNoRecords(uint64_t offset_past_end) {
    ...
    Reader* offset_reader =
        new Reader(&source_, &report_, true, WrittenBytes() + offset_past_end);
    Slice record;
    std::string scratch;
...
uint64_t LogTest::initial_offset_last_record_offsets_[] = {
    ...
    2 * (kHeaderSize + 10000) + (2 * log::kBlockSize - 1000) +
        3 * kHeaderSize,  //  record1 + record2 + 横跨三个块的头部
...
TEST_F(LogTest, MarginalTrailer) {
  ...
  ASSERT_EQ("",
            Read());  // 每次读取一个块缓存，所以文件的指针会比记录的指针更前
  ASSERT_EQ("bar", Read());
  ASSERT_EQ("EOF", Read());
}
...
TEST_F(LogTest, ReadError) {
  Write("foo");
  ForceError();  // 强制使reader报错，读取下一个块失败
  ASSERT_EQ("EOF", Read());
...
TEST_F(LogTest, SkipIntoMultiRecord) {
  ...
  // 如果 initial_offset 指向 first(R1) 之后但在 first(R2) 之前的记录
  // 不完整的片段错误并非实际错误，必须抑制，直到遇到新的 first 或 full 记录。
...
TEST_F(LogTest, ErrorJoinsRecords) {
  ...
  // 其中中间两个片段消失。我们不希望 first(R1) 和 last(R2)
  // 被连接并作为有效记录返回。
