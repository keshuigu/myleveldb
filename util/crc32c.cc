...
// 查找表
// 在 CRC32C 校验和计算中，预计算的查找表（lookup
// table）用于加速校验和的计算过程。 这些查找表包含了所有可能的字节值的 CRC32C
// 结果，因此在计算过程中可以直接查表，而不是每次都重新计算。
...
// 返回大于或等于给定地址且对齐到 N 字节的最小地址。
//
// N 必须是 2 的幂。
template <int N>
constexpr inline const uint8_t* RoundUp(const uint8_t* pointer) {
  // 0x 0100 1001 -> pointer
  // 0x 0010 0000 -> N
  // 0x 0001 1111 -> N - 1
  // 0x 0110 1000 -> pointer + N - 1
  // 0x 1110 0000 -> ~(N - 1)
  // 0x 0110 0000 -> result
...
uint32_t Extend(uint32_t crc, const char* data, size_t n) {
  static bool acclerate = CanAccelerateCRC32C();  // 第一次调用时执行
  if (acclerate) {
